        ...
        <div className="result-section">
          ...
          {Object.entries(
            results.reduce((acc, cur) => {
              acc[cur.filename] = acc[cur.filename] || [];
              acc[cur.filename].push(cur);
              return acc;
            }, {})
          ).map(([filename, detections], index) => (
            <div key={index} className="result-card">
              <h4>📄 Filename: {filename}</h4>

              {files.find(f => f.name === filename) && (
                <img
                  src={URL.createObjectURL(files.find(f => f.name === filename))}
                  alt={filename}
              ...
              {detections.map((item, i) => (
                <div key={i} style={{ marginBottom: "8px" }}>
                  <p><strong>Type:</strong> {item.type} | <strong>Detection ID:</strong> {item.detectionId || "-"}</p>
                </div>
              ))}
            </div>
          ))}
